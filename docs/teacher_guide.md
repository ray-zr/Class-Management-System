...
网页端“小组管理”使用固定 8 列的课室座位表，每个小组区域占两列，组内学生固定按两列排列。拖动小组区域左上角的手柄到另一个小组即可交换位置；调整后会通过 `PUT /api/groups/layout` 保存到数据库，在其他浏览器或设备重新登录后仍保持一致。

学生可直接拖到同组另一名学生上交换座位，也可拖到其他小组的学生块或空白区域调整分组和座位顺序；结果通过 `PUT /api/students/seat` 持久化。触屏设备也可先点选学生，再点击目标小组的空白区域完成分组。未分组学生集中显示在座位画布右侧。讲台位于座位区域下方。
