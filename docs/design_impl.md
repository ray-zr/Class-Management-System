...
- 积分明细不再自动清理。主动撤销时记录撤销时间和必填原因，不物理删除原明细；被撤销记录不参与总分、排行和导出统计。
- `GET /api/operation-logs` 同时返回有效和已撤销的加减分记录，按加减分发生时间或撤销发生时间匹配 `startDate`、`endDate`；网页端默认展示当月。
...
- 主动撤销明细会同步回滚学生累计分并保存原因；软删除学生仍可回滚，升级前已硬删除的学生则保留日志但无法再调整已不存在学生的累计分。
